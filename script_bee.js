...
function Awake()
{
	...
	xMax				= transform.position.x + 0.5;
	xMin					= transform.position.x - 0.5;
...
function moveAndShoot()
{
	...
	if (range >= 0 && range <5)
	{	
		aniPlay.aniSprite(3,4,0,0,6,12);
		if (canAttack)
		{
			canAttack = false;
			attack(attack1);
	...
	{
		aniPlay.aniSprite(3,4,0,2,6,12);
		if (range <= 8 )
		{
			if (transform.position.x > xMin) { transform.position.x -= .01; }//transform.position.y -=.01;	}
		}
		else
		{
			if (transform.position.x < xMax) { transform.position.x += .01; }//transform.position.y -=.01; }
		}	
	}
	
	if (transform.position.y <= 2 && transform.position.y > .75)
	{
		if (range >= 0 && range <=5)
		{
...
{
	clone = Instantiate(power, spawnAttack.position,Quaternion.Euler(0,270,90));
	//canAttack = true;
}
